...
```{r}
Facetscatter2plusno <- df %>%
  ggplot(aes(x = avg_price_usd, y = tot_vol, col = type)) + 
    geom_point(alpha = 0.3) +
    facet_wrap(~region) +
    scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                  labels = trans_format("log10", math_format(10^.x))) +
    labs(title = "2+ Variables: Faceted Scatter Plot with Color", x = "Avg Avacado Price (USD)", y = "Total Volume")


```

```{r More than Two Variables, echo = FALSE, fig.width=8, fig.height=4.5, dpi=300, out.width="800px", out.height="529px"}
